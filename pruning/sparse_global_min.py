...
class Strategy(base.Strategy):
    ...
    def sparse_global(model: Model, current_mask: Mask, args) -> Mask:
        ...
        thereshold = np.sort(np.abs(weight_vector))[::-1][number_of_weights_to_prune]

        new_mask = Mask({k: np.where(np.abs(v) < thereshold, current_mask[k], np.zeros_like(v)) for k, v in weights.items()})
